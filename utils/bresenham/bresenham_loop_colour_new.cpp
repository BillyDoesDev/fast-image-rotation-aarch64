...
// Function to rotate a single channel of an image by a given angle using backward mapping
...
cv::Mat rotateImage(const cv::Mat& image, double angle) {
    ...
    std::vector<cv::Mat> rotatedChannels(channels.size());
    std::vector<std::future<cv::Mat>> futures;

    for (size_t i = 0; i < channels.size(); ++i) {
        futures.push_back(std::async(std::launch::async, rotateChannel, channels[i], angle));
    }

    for (size_t i = 0; i < channels.size(); ++i) {
        rotatedChannels[i] = futures[i].get();
    }
...
int main(int argc, char *argv[]) {
    ...
    double angle = std::stod(argv[3]); // Rotate by 120 degrees
